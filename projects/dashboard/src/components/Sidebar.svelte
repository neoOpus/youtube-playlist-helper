  ...
  <nav class="sidebar-nav">
    <div class="nav-group">
      ...
      <button class="nav-link" class:active={isActive("/new")} onclick={() => router.push("/new")}>
        ...
        <span class="link-text">Add New</span>
    ...
    <div class="nav-group">
      ...
      <button class="nav-link" class:active={isActive("/sync")} onclick={() => router.push("/sync")}>
        ...
        <span class="link-text">Cloud Sync</span>
...
<style>
  ...
  .nav-link {
    display: flex; align-items: center; gap: 12px; padding: 12px 16px; border-radius: 10px; border: none; background: transparent; color: var(--text-secondary); cursor: pointer; transition: all 0.2s; text-align: left; width: 100%; font-weight: 600; font-size: 0.9rem; position: relative;
  }
  ...
  .active-indicator { position: absolute; left: 4px; top: 12px; bottom: 12px; width: 3px; background: var(--primary); border-radius: 4px; }

  .sidebar-footer { padding: 16px 12px; border-top: 1px solid var(--border-base); }

  @media (max-width: 1000px) {
    .link-text, .group-label, .logo-text, .active-indicator { display: none; }
