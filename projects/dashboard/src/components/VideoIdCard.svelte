    ...
    <div class="modal-content pro-glass-high" transition:fly={{ y: 20 }} onclick={e => e.stopPropagation()}>
        ...
        <div class="modal-body">
            <div class="meta-section">
                ...
                <input bind:value={video.title} placeholder="Unknown Metadata..." />
            </div>

            <div class="grid-2 mt-6">
                <div class="meta-section">
                    <span class="label">RATING</span>
                    <select bind:value={video.rating}>
            ...
            <div class="meta-section mt-6">
                ...
                <textarea bind:value={video.notes} placeholder="Enter observations..."></textarea>
            </div>
        </div>

        <div class="modal-footer mt-8">
            <SuperButton onclick={save} class="w-full">
...
<style>
    ...
    .modal-content { width: 500px; max-width: 90vw; padding: 2.5rem; position: relative; }
    .modal-header { display: flex; align-items: center; gap: 1rem; margin-bottom: 2rem; }
    h2 { margin: 0; font-weight: 900; flex-grow: 1; letter-spacing: -1px; }
    ...
    input, select, textarea { background: var(--bg-secondary); border: 1px solid var(--border); border-radius: 8px; padding: 12px; color: var(--text); font-weight: 600; font-size: 0.9rem; }
